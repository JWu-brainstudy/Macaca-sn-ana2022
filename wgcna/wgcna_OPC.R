...
#Extracting OPC expression matrix
data1 <- readRDS("snRNA.2.rds")
Idents(data1) = data1$Celltype
x1 <- SplitObject(data1,split.by = "ident")[["OPC"]]
...
ggsave("sampleClustering_OPC.png",height = 9,width = 12)
...
ggsave("soft-thresholding power_OPC.png",height = 6,width = 12)
...
ggsave("cluster Dendrogram_OPC.png",height = 9,width = 12)
...
ggsave("cluster-dynamic Dendrogram_OPC.png",height = 9,width = 12)
...
write.xlsx(meta2module, "meta2module_OPC.xlsx", overwrite = T, rowNames = T)
nGenes = ncol(x);
nSamples = nrow(x);
...
p.orign
ggsave("wgcna_OPC.pdf", width = 12, height = 5)
ggsave("wgcna_OPC.png", width = 12, height = 5)
