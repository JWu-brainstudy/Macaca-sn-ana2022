...
#Extracting Ex expression matrix
data1 <- readRDS("snRNA.2.rds")
Idents(data1) = data1$Celltype
x1 <- SplitObject(data1,split.by = "ident")[["Ex"]]
...
ggsave("sampleClustering_Ex.png",height = 9,width = 12)
...
ggsave("soft-thresholding power_Ex.png",height = 6,width = 12)
...
ggsave("cluster Dendrogram_Ex.png",height = 9,width = 12)
...
ggsave("cluster-dynamic Dendrogram_Ex.png",height = 9,width = 12)
...
write.xlsx(meta2module, "meta2module_Ex.xlsx", overwrite = T, rowNames = T)
nGenes = ncol(x);
nSamples = nrow(x);
...
p.orign
ggsave("wgcna_Ex.pdf", width = 12, height = 5)
ggsave("wgcna_Ex.png", width = 12, height = 5)
